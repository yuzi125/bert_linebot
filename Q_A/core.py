...
def convert_data_to_feature(tokenizer, train_data_path):
    ...
    for q in question_dic.data:
        ...
        if(len(bert_ids)>max_seq_len):
            ...
            max_seq_num = len(question_dic.data)
...
def convert_data_to_feature_csv(tokenizer, train_data_path):
    with open(train_data_path,'r',encoding='utf-8',newline='') as csvfile:
    ...
    output = open('bert_part/trained_model/data_features.pkl', 'wb')
    pickle.dump(data_features,output)
    return data_features
